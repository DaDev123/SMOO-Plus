...
inline s32 BufferedSafeStringBase<T>::copy(const SafeStringBase<T>& src, s32 copyLength) {
    ...
    if (copyLength >= mBufferSize) {
        SEAD_ASSERT_MSG(false, "Buffer overflow. (Buffer Size: %d, Copy Size: %d)", mBufferSize, copyLength);
...
inline s32 BufferedSafeStringBase<T>::copyAt(s32 at, const SafeStringBase<T>& src, s32 copyLength) {
    ...
    if (copyLength >= mBufferSize - at) {
        SEAD_ASSERT_MSG(false, "Buffer overflow. (Buffer Size: %d, At: %d, Copy Length: %d)", mBufferSize, at,
                        copyLength);
...
template <typename T>
inline s32 BufferedSafeStringBase<T>::cutOffCopyAt(s32 at, const SafeStringBase<T>& src, s32 copyLength) {
    T* dst = getMutableStringTop_();
    s32 len = this->calcLength();
...
inline s32 BufferedSafeStringBase<T>::append(T c) {
    ...
    if (length >= getBufferSize() - 1) {
        SEAD_ASSERT_MSG(false, "Buffer overflow. (Buffer Size: %d, Length: %d)", getBufferSize(), length);
        return 0;
    }
...
inline s32 BufferedSafeStringBase<T>::append(T c, s32 num) {
    ...
    if (length >= getBufferSize() - num) {
        SEAD_ASSERT_MSG(false, "Buffer overflow. (Buffer Size: %d, Length: %d, Num: %d)", getBufferSize(),
                        length, num);
...
inline s32 BufferedSafeStringBase<T>::chop(s32 chop_num) {
    s32 length = this->calcLength();
    T* buffer = getMutableStringTop_();
    const auto fail = [=] { SEAD_ASSERT_MSG(false, "chop_num(%d) out of range[0, %d]", chop_num, length); };
...
inline s32 BufferedSafeStringBase<T>::replaceCharList(const SafeStringBase<T>& old_chars,
    ...
    if (old_chars_len != new_chars_len) {
        ...
        SEAD_ASSERT_MSG(false, "old_chars(%s).length is not equal to new_chars(%s).length.", old_chars.cstr(),
                        new_chars.cstr());
        if (old_chars_len > new_chars_len)
...
inline s32 BufferedSafeStringBase<T>::setReplaceString(const SafeStringBase<T>& target_str,
    ...
    const s32 ret = replaceStringImpl_(getMutableStringTop_(), nullptr, getBufferSize(), target_str.cstr(),
...
inline s32 BufferedSafeStringBase<T>::replaceString(const SafeStringBase<T>& old_str,
                                                    const SafeStringBase<T>& new_str) {
    bool is_buffer_overflow = false;
    const s32 ret = replaceStringImpl_(getMutableStringTop_(), nullptr, getBufferSize(), this->cstr(),
                                       this->calcLength(), old_str, new_str, &is_buffer_overflow);
    SEAD_ASSERT_MSG(!is_buffer_overflow, "Buffer overflow! (%s(replacing) : s/%s/%s/g, Buffer Size: %d )",
                    this->cstr(), old_str.cstr(), new_str.cstr(), getBufferSize());
    return ret;
}
...
inline s32 BufferedSafeStringBase<T>::convertFromOtherType_(const SafeStringBase<OtherType>& src,
    ...
    if (getBufferSize() <= copy_size) {
        SEAD_ASSERT_MSG(false, "copy_size(%d) out of bounds[%d, %d)", copy_size, 0, getBufferSize());
