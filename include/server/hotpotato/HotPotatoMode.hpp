...
struct PACKED HotPotatoPacket : Packet {
    ...
    HotPotatoUpdateType updateType;
    bool isRunner = false;
    bool isFreeze = false;
    uint16_t score = 0;
};
...
private:
    ...
    // Scene actors
    HotPotatoPlayerBlock* mMainPlayerIceBlock = nullptr; // Visual block around player's when frozen
    HotPotatoHintArrow* mHintArrow = nullptr; // Arrow that points to nearest runner while in chaser
