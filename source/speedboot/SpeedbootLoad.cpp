...
const char* SpeedbootLoad::getStageName() const {
    ...
    if (!mSequence->stageName.isEmpty()) {
        return mSequence->stageName.cstr();
    }

    // Try next stage name
    const char* name = GameDataFunction::getNextStageName(mSequence->mGameDataHolder);
    if (name && name[0] != '\0') return name;

    // Fallback to current stage
    return GameDataFunction::getCurrentStageName(mSequence->mGameDataHolder);
}
