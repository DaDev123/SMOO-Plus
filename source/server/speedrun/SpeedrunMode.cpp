#include "server/speedrun/SpeedrunMode.hpp"
...
#include "server/speedrun/SpeedrunPacket.hpp"
...
void SpeedrunMode::processPacket(Packet* _packet) {
    SpeedrunPacket* packet     = (SpeedrunPacket*)_packet;
...
bool SpeedrunMode::showNameTag(PuppetInfo* other) {
    return (
...
void SpeedrunMode::debugMenuControls(sead::TextWriter* gTextWriter) {
...
void SpeedrunMode::updateTagState(bool isSeeking) {
...
void SpeedrunMode::onBorderPullBackFirstStep(al::LiveActor* actor) {
    if (isUseGravity()) {
        killMainPlayer(actor);
    }
}

void SpeedrunMode::createCustomCameraTicket(al::CameraDirector* director) {
