...
HotPotatoConfigMenu::HotPotatoConfigMenu() : GameModeConfigMenu() {
...
void HotPotatoConfigMenu::initMenu(const al::LayoutInitInfo &initInfo) {}

const sead::WFixedSafeString<0x200> *HotPotatoConfigMenu::getStringData() {
...
bool HotPotatoConfigMenu::updateMenu(int selectIndex) {

    HotPotatoInfo *curMode = GameModeManager::instance()->getInfo<HotPotatoInfo>();
    ...
    switch (selectIndex) {
        case 0: {
            if (GameModeManager::instance()->isModeAndActive(GameMode::HOTPOTATO)) {
                uint16_t oldScore = GameModeManager::instance()->getInfo<HotPotatoInfo>()->mPlayerTagScore.mScore;
                uint16_t newScore = -1;
