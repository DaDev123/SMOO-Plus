...
SocketClient::SocketClient(const char* name, sead::Heap* heap) : mHeap(heap), SocketBase(name) {
    mRecvThread = new al::AsyncFunctorThread(
        "SocketRecvThread",
        al::FunctorV0M<SocketClient*, SocketThreadFunc>(this, &SocketClient::recvFunc), 0, 0x1000,
        {0});
    mSendThread = new al::AsyncFunctorThread(
        "SocketSendThread",
        al::FunctorV0M<SocketClient*, SocketThreadFunc>(this, &SocketClient::sendFunc), 0, 0x1000,
        {0});
...
nn::Result SocketClient::init(const char* ip, u16 port) {
    ...
    nn::socket::SetSockOpt(this->socket_log_socket, 6, TCP_NODELAY, &sockOptValue,
                           sizeof(sockOptValue));

    nn::Result result;

    if ((result = nn::socket::Connect(this->socket_log_socket, (sockaddr*)&serverAddress,
...
bool SocketClient::send(Packet* packet) {
    ...
    if ((valread = nn::socket::Send(this->socket_log_socket, buffer,
                                    packet->mPacketSize + sizeof(Packet), 0) > 0)) {
        return true;
    } else {
    ...
    if (valread > 0) {
        ...
        if (header->mType > PacketType::UNKNOWN && header->mType < PacketType::End &&
            ...
            if (packetBuf) {
                ...
                if (!!(mRecvQueue.mMessageQueueInner._count ==
                       mRecvQueue.mMessageQueueInner._maxCount)) {
...
Packet* SocketClient::tryGetPacket() {
    return socket_log_state == SOCKET_LOG_CONNECTED ?
