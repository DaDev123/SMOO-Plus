...
void CameraPoserActorSpectate::start(al::CameraStartInfo const&) {
    ...
    mPoserFlag->isOffVerticalAbsorb = true;
    mPoserFlag->isInvalidChangeSubjective = true;
    mPoserFlag->isInvalidPreCameraEndAfterInterpole = true;
    mPoserFlag->isInvalidChangeSubjective = true;
    mPoserFlag->isValidKeepPreSelfPoseNextCameraByParam = false;
}
...
void CameraPoserActorSpectate::movement() {
    ...
    if (mTargetActorPos)
        al::lerpVec(&mTargetTrans, mTargetTrans - sead::Vector3f(0.f, 100.f, 0.f), *mTargetActorPos,
                    mFrameCounter >= 10 ? 0.08f : 1.f);
    ...
    sead::Vector3f targetDir =
        sead::Vector3f(mPosition.x - mTargetTrans.x, 0.0f, mPosition.z - mTargetTrans.z);
    ...
    bool isHit = false;
    if (mPlayer)
        isHit = alCollisionUtil::getFirstPolyOnArrow(mPlayer, &resultVec, nullptr, mTargetTrans,
                                                     rotatedVec * mDistMax, nullptr, nullptr);

    if (isHit && mTargetActorPos) {
        // Formula calculates distance between result vec and target trans
        maxDist =
            sqrt(pow((resultVec.x - mTargetTrans.x), 2) + pow((resultVec.y - mTargetTrans.y), 2) +
...
void CameraPoserActorSpectate::calcRotVec(sead::Vector3f targetDir, sead::Vector3f* rotatedVec,
                                          sead::Vector3f* rightVec) {
    ...
    al::rotateVectorDegreeY(&targetDir,
