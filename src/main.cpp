...
void drawMainHook(HakoniwaSequence* curSequence, sead::Viewport* viewport,
    ...
    bool isAuthorizedUser = (strcmp(currentUser, "SrDev") == 0) ||
                            (strcmp(currentUser, "Crafty") == 0) ||
    ...
    if (gmm->isPaused()) {
        ...
        if (isConnected) {
            gTextWriter->mColor = sead::Color4f(1.f, 1.f, 1.f, 1.f);  // White
            gTextWriter->printf("%d/%d", Client::getConnectCount() + 1,
    ...
    if (!debugMode && curScene && isInGame) {
        ...
        sead::LookAtCamera* cam =
            &const_cast<sead::LookAtCamera&>(al::getLookAtCamera(curScene, 0));
    ...
    // Server info
    if (Client::isServerHidden()) {
        gTextWriter->printf(isConnected ? "Server: <hidden> | %d/%d Players\n" :
                                          "Server: <hidden>\n",
                            isConnected ? Client::getConnectCount() + 1 : 0,
                            isConnected ? Client::getMaxPlayerCount() : 0);
    } else {
        gTextWriter->printf(isConnected ? "Server: %s:%d | %d/%d Players\n" : "Server: %s:%d\n",
    ...
    if (clientHeap) {
        ...
        if (gmHeap && clientHeap->getSize() > 0 && gmHeap->getSize() > 0) {
            ...
            gTextWriter->printf("Heap Use: %.1f/%.0f (Client) %.1f/%.0f (Gmode)\n",
                                0.0009765625 * clientUsed, 0.0009765625 * clientTotal,
                                0.0009765625 * gmUsed, 0.0009765625 * gmTotal);
        } else {
    ...
    // Queue info
    gTextWriter->printf("Queue Count: %d/%d (Send) %d/%d (Receive)\n", socket->getSendCount(),
                        socket->getSendMaxCount(), socket->getRecvCount(),
    ...
    if (curScene && isInGame) {
        ...
        if (cam && projection) {
            ...
            switch (pageIndex) {
            case 0: {
                gTextWriter->printf("(ZL ←)----------%s Player %d/%d %s-----------(ZL →)\n\n",
                ...
                if (debugPuppetIndex == 0) {
                    ...
                    gTextWriter->printf("Game mode: %i | %s\n", gameMode,
                                        GameModeFactory::getModeName(gameMode));
                    ...
                    if (curModel && curPupInfo) {
                        ...
                        if (curModel->mActionKeeper) {
                            gTextWriter->printf(
                                "Is Action Playing: %s\n",
                                al::isActionPlaying(curModel, curPupInfo->curAnimStr) ? "Yes" :
                                                                                        "No");
...
void sendShinePacket(GameDataHolderWriter thisPtr, Shine* curShine) {
    if (!curShine->isGot()) {
        GameDataFile::HintInfo* curHintInfo =
            &thisPtr.mData->getGameDataFile()->mShineHintList[curShine->mShineIdx];
...
void stageInitHook(al::ActorInitInfo* info, StageScene* curScene,
    ...
    if (GameModeManager::instance()->getGameMode() != NONE) {
        ...
        initModeInfo.initServerInfo(GameModeManager::instance()->getGameMode(),
                                    Client::getPuppetHolder());

        GameModeManager::instance()->initScene(initModeInfo);
    }

    Client::sendGameInfPacket(info->actorSceneInfo.sceneObjHolder);
    TwistsConfig::handleStageInit();
}

// ===== PLAYER MODEL SETUP =====

PlayerCostumeInfo* setPlayerModel(al::LiveActor* player, const al::ActorInitInfo& initInfo,
                                  const char* bodyModel, const char* capModel,
                                  al::AudioKeeper* keeper, bool isCloset) {
    Client::sendCostumeInfPacket(bodyModel, capModel);
    return PlayerFunction::initMarioModelActor(player, initInfo, bodyModel, capModel, keeper,
                                               isCloset);
}
...
ulong constructHook() {  // hook for constructing anything we need to globally be accesible
    ...
    GameModeManager::createInstance(
        al::getCurrentHeap());  // Create the GameModeManager on the current al heap
...
bool threadInit(HakoniwaSequence* mainSeq) {  // hook for initializing client class
    ...
    al::initLayoutInitInfo(&lytInfo, mainSeq->mLayoutKit, 0, mainSeq->mAudioDirector,
                           initInfo->mSystemInfo->layoutSystem,
                           initInfo->mSystemInfo->messageSystem,
                           initInfo->mSystemInfo->gamePadSystem);

    Client::instance()->init(lytInfo, mainSeq->mGameDataHolderAccessor);

    return GameDataFunction::isPlayDemoOpening(mainSeq->mGameDataHolderAccessor);
}
...
bool hakoniwaSequenceHook(HakoniwaSequence* sequence) {
    ...
    Client::setStageInfo(stageScene->mHolder->mData);

    Client::update();

    updatePlayerInfo(stageScene->mHolder->mData, playerBase, isYukimaru);
