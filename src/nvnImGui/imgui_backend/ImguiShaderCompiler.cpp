...
//    Logger::log("Allocating ptr with size: %x aligned by %x\n", size, alignment);
    return nn::init::GetAllocator()->Allocate(nn::util::align_up(size, alignment));
}
...
void __attribute__((noinline)) ReadCompiledShader(GLSLCoutput *compileData) {
...
CompiledData __attribute__((noinline)) CreateShaderBinary(GLSLCoutput *compileData, const char *shaderName, bool outputFile = true) {
    ...
    for (int i = 0; i < compileData->numSections; ++i) {
        if (compileData->headers[i].genericHeader.common.type == GLSLCsectionTypeEnum::GLSLC_SECTION_TYPE_GPU_CODE) {
            binarySize = nn::util::align_up(binarySize + compileData->headers[i].genericHeader.common.size, 0x100);
        }
    }

    binarySize = nn::util::align_up(binarySize + 0x1000, 0x1000);
    ...
    for (int i = 0; i < compileData->numSections; ++i) {
        ...
        curBinaryPos = nn::util::align_up(curBinaryPos + compInfo->controlSize, 0x100);
    }
    ...
    for (int i = 0; i < compileData->numSections; ++i) {
        ...
        curBinaryPos = nn::util::align_up(curBinaryPos + compInfo->dataSize, 0x100);
    }
    ...
    if (outputFile) {
        ...
        char fullPath[0x40] = {};
        createPath(fullPath, "content:/ShaderData", shaderName, ".bin");

        HK_ABORT_UNLESS_R(FsHelper::writeFileToPath(binaryBuffer, binarySize, fullPath).IsSuccess());
...
const char *GetShaderSource(const char *path) {
    ...
    HK_ASSERT(FsHelper::isFileExist(path));//, "Failed to Find File!"

    HK_ABORT_UNLESS_R(nn::fs::OpenFile(&handle, path, nn::fs::OpenMode_Read).IsSuccess());

    long size = 0;
    nn::fs::GetFileSize(&size, handle);
    char *sourceFile = (char *) glslc_Alloc(size + 1, 8);

    HK_ABORT_UNLESS_R(nn::fs::ReadFile(handle, 0, sourceFile, size).IsSuccess());
...
CompiledData ImguiShaderCompiler::CompileShader(const char *shaderName) {
    ...
    createPath(vshPath, "content:/ShaderData/Sources", shaderName, "_vsh.glsl");
    char fshPath[0x50] = {}; //"sd:/smo/shaders/sources/imgui_fsh.glsl";
    createPath(fshPath, "content:/ShaderData/Sources", shaderName, "_fsh.glsl");
